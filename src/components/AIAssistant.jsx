...
export default function AIAssistant() {
  ...
  async function send() {
    ...
    setLoading(true) // Show the three animated dots

    try {
      const res = await fetch('/api/chat', {
        // Same-origin API route implemented in Next.js (or use standalone server.js in class exercises)
        method: 'POST', // We are creating a new “question” resource on the server
        headers: { 'Content-Type': 'application/json' }, // Tell the server the body is JSON
        body: JSON.stringify({
          message: userMessage, // Latest user text only
          conversationHistory, // Full prior context (includes opening assistant line)
        }),
      })
      const data = await res.json() // Parse JSON body from the server
      let reply = 'Sorry, something went wrong. Please try again.' // Default if response is unexpected
      if (res.ok && typeof data.message === 'string') {
        reply = data.message // Normal assistant text from Claude
      } else if (typeof data.message === 'string' && data.message.length > 0) {
        reply = `Sorry, something went wrong. ${data.message}` // Server sent an error detail (e.g. invalid model)
      } else if (typeof data.error === 'string' && data.error.length > 0) {
        reply = `Sorry, something went wrong. (${data.error})` // Fallback error field
      }
      setMessages((prev) => [...prev, { role: 'assistant', content: reply }]) // Append AI bubble
    } catch {
      setMessages((prev) => [
        ...prev,
        {
      ...
      ]) // Network or server failure path
    }

    setLoading(false) // Hide typing dots whether success or failure
  }
          ...
          {!chatOpen ? (
            ...
            <div className={styles.widget}>
              ...
              <div className={styles.messages} role="log" aria-live="polite">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={isUser ? styles.bubbleUser : styles.bubbleAi}>
                        {/* Grey bubble for AI, teal for user */}
                        {msg.content.split('\n').map((line, lineIndex, arr) => (
                          // Support multi-line answers from the model
                          <span key={lineIndex}>
                            {line}
                ...
                {/* End typing indicator conditional */}
